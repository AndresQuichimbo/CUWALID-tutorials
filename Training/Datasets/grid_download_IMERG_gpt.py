...
def find_available_version(session, base_url, yyyy, doy, run_char, start_str, end_str, delta_time, url_aggregation):
	"""
	Pings NASA's server to check whether V07B, V07C, or another version is present.
	Returns the full, valid URL and the correct filename if found; otherwise returns None.
	"""
	# Test letters sequentially: B, C, D, A
	for letter in ['B', 'C']:#, 'D', 'A']:
		filename = f"3B-HHR{run_char}.MS.MRG.3IMERG.{start_str}-{end_str}.{delta_time:04}.V07{letter}.HDF5.nc4"
		test_url = f"{base_url}/{yyyy}/{doy}/{filename}{url_aggregation}"
		#print(f"Testing URL: {test_url}")
		try:
			# HEAD requests are fast because they only fetch headers, not the full dataset
...
for i, istart_date in enumerate(dates_list[:-1]):
	...
	while current <= iend_date:
		...
		if (PRODUCT_TYPE == 'Early') & (int(doy) > 62):
			filename  = f"3B-HHR{run_char}.MS.MRG.3IMERG.{start_str}-{end_str}.{delta_time:04}.V07C.HDF5.nc4"
...
#	# =========================================================================
#	# REPLACED LOOP IN YOUR MAIN CODE
#	# =========================================================================
#	delta_time = 0
#	current = istart_date
#	while current <= iend_date:
...
#		end_str = end_str.strftime("E%H%M59")
#	
#		# Dynamically query NASA for the correct version variant
#		url, filename = find_available_version(
#			session, base_url, yyyy, doy, run_char, start_str, end_str, delta_time, url_aggregation
#		)
#	
#		if url is None:
#			print(f"❌ No valid V07 version variant found for {start_str}")
#			current += timedelta(minutes=30)
#			# Reset delta time matching your script's logic
#			delta_time += 30
#			if delta_time > 1410: delta_time = 0
#			continue	
#		local_path = os.path.join(output_dir, "raw", filename)
#		os.makedirs(os.path.dirname(local_path), exist_ok=True)	
#		try:
...
#							f.write(chunk)
#				else:
#					print(f"❌ {filename} Download Failed ({r.status_code})")
#					current += timedelta(minutes=30)
#					delta_time += 30
#					if delta_time > 1410: delta_time = 0
#					continue	
